...
> **消费点**：扫榜域 Stage 2 采集与质检时按需加载
> 来源：移植自 mo-shu v2.6.1
> **边界**：管采集字段、输出模板与清洗规则；平台命令入口归 collection-guide
...
榜单清单与 URL 见 collection-guide.md「起点采集目标」节。
...
榜单 URL 格式与参数说明见 collection-guide.md「番茄采集目标」节。
...
- 控制台若报 `CDP 无响应`，说明 Chrome/CDP 没起来或端口不对，按 cdp-base.md 重新启动。确认正常后重采。
