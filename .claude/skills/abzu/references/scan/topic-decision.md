...
2. **下游可达性约束**：开书时（outline 域 开书）从项目根及其上一级目录起、向下最多 3 层按文件名搜 `选题决策.md`——本约定目录恰好落在边界内；小说项目建得比 3 层更深时，用户开书前需把 `选题决策.md` 复制到项目根（或开书时把路径粘给 outline 域）。
...
4. **拆文保持独立，不自动衔接**：提示一句「想坐实'能爆的原因'可自行 analyze 域 拆 {对标书}，拆完回填」即可——拆不拆、何时拆由用户决定，选定选题不代表进入拆文。
5. 告知用户路径，并说明下一步：「开书时如果小说项目就建在这个目录里、或和这个目录同级（共用一个上级目录），outline 域 会自动找到并让你确认；隔得更远就把 `选题决策.md` 复制到小说项目根目录，或开书时把路径粘给它。」
