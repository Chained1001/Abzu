...
function scrapeRank(port, rankTypeId, channelId) {
  ...
  try {
    ...
    if (!probe) {
      console.error(
        `  ✗ CDP 无响应。请确认已用 setup-cdp-chrome.js 启动 Chrome（端口 ${port}），且 agent-browser 可用。`
      );
      return null;
    }
  ...
  if (!LIST_ONLY) {
    ...
    if (picked.length) {
      ...
      // （下面的质量门会把 detailOk===0 标成 [存在问题]，问题摘要写 [详情解析异常/登录态缺失]）
      try {
  ...
  // 质量状态：详情开启时，收藏数命中率是核心信号（数据质量枚举统一为 [OK/存在问题]，具体问题进问题摘要）
  const detailPartial =
    !LIST_ONLY &&
    detailPlanned > 0 &&
  ...
  if (!LIST_ONLY && detailPlanned > 0 && detailOk === 0) {
    problems.push("[详情解析异常/登录态缺失] 全部详情无收藏数：可能页面结构变动或需登录");
    console.error(`  ⚠ 详情全部无收藏数：可能页面结构变动或需登录，已在文件头标注。`);
  } else if (detailPartial) {
    problems.push(`[部分详情缺失] 详情仅命中 ${detailOk}/${detailPlanned}`);
