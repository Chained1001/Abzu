...
8. **分工协作（规划方/施工方）**：非平凡改动可采用两窗口分工——**规划方**（持全局上下文）勘察现状、写施工规格（docs/specs/，自包含：现状事实/文件级改动清单/禁止事项/验收标准）、产出施工提示词；**施工方**（另一窗口/模型）先读 AGENTS.md 与规格再施工，**禁止 git 写操作、禁止扩大范围、禁止改 CHANGELOG**，规格与现状不符时停下记偏差；完成后输出【核验请求】（改动清单对照规格/验收命令自跑结果/偏差记录/未决问题）。**规划方核验三步**：git diff 对照清单、验收命令独立重跑、内容双轨抽查（§3.6）——结论三态：通过提交（作者确认）/整改（出整改提示词）/裁决（呈作者）。**铁律：施工规格写完必须先自审一遍**（现状锚点与实文一致/清单完备可执行/验收可判定/内部无矛盾），自审通过才准产出施工提示词。（2026-09-06 立例：首份爬虫规格自审即抓到礼仪上限与脚本默认冲突、锚点不逐字两处。）
