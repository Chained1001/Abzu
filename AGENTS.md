...
- 一键检查（提交前必跑，pre-commit hook 自动执行）：`bash scripts/check.sh`——[0] skills 真目录守卫（安装器 symlink 化检测）＋[1] 六壳 validate ＋[2] markdownlint ＋[3] 内容轨（引用闭合/加粗密度/TOC/SKILL.md 行数/CHANGELOG 条目限长）＋[4] 规格静态自检（在制规格的断言自噬预警与计数重算）＋[5] 脚本语法检查（node --check）；agentskills 与 Python 探测链固化于该脚本，新增检查项的接入位置见脚本头注「维护入口」
...
8. **分工协作（规划方/施工方/作者）**：非平凡改动（判定表 ✅ 行）适用本条协作协议；平凡改动按本节 1-3 直接执行（原「大型」前置门槛退役，2026-09-07 外审裁定——门槛退役由规格化协作本身支撑，不依赖具体施工形态）。
    ...
    - **协议细则单点权威在 collab-log**：全套提示词与回传模板、P0/P1 偏差分级与 findings 处置、规划方核验三步与结论三态、审查工序（成稿/产物）、施工记录表——见 docs/specs/collab-log.md；协作记录逐次登记（规格/施工方模型/自由度分布/偏差次数/整改轮数/结果）。（细则下沉系反模式 #4 去重——原条文与 collab-log 核验协议重复。）
...
3. 必须新增时：**验收标准**是什么？**索引**（`docs/` 内相关文档）是否同步更新？→ 缺一不合并。
...
| 2 | 删改机制留文档残留（或反之） | 当次全仓 grep 清理全部引用；**grep 词须含概念近义词不止原句**（032 立法只搜原句，姊妹文档「触发关键词」措辞漏网至次日审计——2026-09-10 复盘立例） | 继承（mo-shu） |
