...
> **本文件定位**：仓库唯一宪法——红线、分工原则、开发节奏、决策流程、不做清单。工程细则见 [docs/standards/ 文件命名规范](docs/standards/file-conventions.md)；产品语言叫法以 [docs/术语表](docs/glossary.md) 为权威。流程术语（candidate、文件账本律、P0/P1、自由度分级等）均在首次出现处内联定义。
...
1. **非平凡改动**（新增脚本/reference/守卫、目录迁移、流程规则变更等跨文件改动；反例：错别字、单文件措辞微调＝平凡）：先探索现状 → 写轻量规格存 `docs/specs/YYYY-MM-DD-<主题>.md`（三要素：现状事实、文件级改动清单、验收标准）→ 用户确认 → 实施 → 验收 → 请求提交。
   ...
   **外审驱动的修改**：外审报告须转为正式规格（docs/specs/），逐条标注采纳/拒绝及理由——禁止拿到外审直接改文件。**外部参考研究**（点名研读仓库/规范）同款处理，研究程序与裁定表用 collab-log 第七件套。（2026-09-06 立例：连续 5 批外审驱动的修改未走规格流程；2026-09-07 补例：015 号 mattpocock/skills 研究经作者提醒才转规格。）
...
**审计深度分级**：核验深度映射施工自由度——[A] 级改动逐字核验（与规格文本 diff 为零）、[B] 级改动通读核验（语义通顺、行为不变）、[C] 级改动按验收标准核验；存量/知识文件最低为 D3 模式扫描（残留词/格式/路径闭合）。**扫描只能证伪不能证真**——零命中仅表示"已知模式无违例"；扫出问题即升级该文件深度，待交付产物模板一律逐字。（2026-09-06 立例：矩阵 9/9 CLEAN 后仍需逐文件通读补审——深度此前无成文标准。）
7. **修复后自核验**：问题修复完成必须自核验三项——①修改已落实到位 ②无遗漏点 ③无衍生新问题；核验通过才算修复完成。（2026-09-05 立例：修复引入双空格与 CHANGELOG 游离行，靠 lint 与事后复核才抓回。）
8. **分工协作（规划方/施工方/作者）**：仅**大型非平凡改动**适用本分工，普通小改动按本节 1-3 直接执行；两窗口分工——**规划方**（持全局上下文）负责勘察、写规格、生成施工提示词、核验、统一提交；**作者**负责确认规格、确认提交、重大分歧拍板；**施工方**（另一窗口/模型）只按规格施工，无 git 写权限。施工提示词必含：先读 AGENTS.md 与规格全文；严格按改动清单施工（每条改动标自由度：[A] 逐字执行 / [B] 方向执行不得改接口行为 / [C] 只给验收标准）；偏差分两级——P0 阻塞型（规格与现状冲突，继续会做错）立即停、输出改为偏差报告，P1 观察型（有出入不影响施工）继续施工、记入核验请求；完成后输出【核验请求】（改动清单对照规格/验收自跑结果/偏差记录/未决问题；整段包单个代码块便于原样回传）——施工提示词一律从 collab-log 模板派生，不得现场自由撰写而漏带格式与偏差分级要求（2026-09-07 立例：014 分身提示词漏带输出格式要求，分身返回未包代码块）。**铁律：施工规格写完必须先自审一遍**（现状锚点与实文一致/清单完备可执行/验收可判定/内部无矛盾），自审通过才准产出施工提示词；写规格时现状锚点必须**现场 grep/read 确认**（文件存在性、内容片段），不得沿用对话记忆——锚点写于前批改动落盘之前就会过时（2026-09-07 立例：014 号规格 3 处现状事实过时，分身施工时才暴露）。**规划方核验三步**：① git diff 对照改动清单（多改/漏改即偏差）② 验收命令独立重跑 ③ 内容双轨抽查（§3.6）——结论三态：通过提交（作者确认）/ 整改（附整改提示词模板）/ 裁决（按【裁决请求】格式呈拍板）。**施工分身模式**（与外部分工并行可选）：规划方可通过 Agent 工具 spawn 施工分身，传规格路径与施工约束，分身执行后返回核验请求——适用于迭代速度优先的场景；规格获作者确认后规划方**主动 spawn**，不退回"作者粘贴到另一窗口"的人工中转（2026-09-07 立例：014 首轮规划方产出粘贴式提示词，作者纠正后才切换分身）。作者角色从"中转"升级为"纯裁决"（仅在规格确认与提交确认时出手）。分身提示词模板见 collab-log。协作记录：docs/specs/collab-log.md 逐次登记（规格/施工方模型/自由度分布/偏差次数/整改轮数/结果），协作模板八件套（施工/核验请求/整改/裁决/审计结论/施工分身/参考研究裁定表/独立审查分身提示词）见该文件头部。（2026-09-06 作者审定。）
9. **立法与修订质量**：新增规范条目必须至少附一个正例或反例——无实例的条文标注「预立法，随首批实例校准」（立法先于实例的条文是猜测，2026-09-06 引号形态歧义立例）；架构/理念级变更的涟漪面除文件路径外，**必须 grep 旧概念词**（如中央路由/总控/单技能——理念表述层的过时不会被路径 grep 捕获，2026-09-06 skill-writing 中央路由残留立例）。
