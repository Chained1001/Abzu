...
> 版本：v1.7（沿革见 CHANGELOG 与 git 历史）
...
**决定性判据：六域共享同一份可变状态（写作项目的 `基本规格.md` / `创作进度.md` / 手稿）**——共享资产的归属方式是两方案的分水岭。
...
v1 无部署器：没有 hooks / agents / 项目级 CLAUDE.md 需要物化，安装即全部部署；书项目脚手架由立项流程自建；版本兼容机制待新书项目设计定义（原版本兼容机制已随 036 批退役，见 release-and-versioning §四）。skills.sh 安装只携带 skill 文件夹（不含 `.claude/commands/` 薄壳）——六域 skill 本名即命令，多域已就位（四占位 + 两实建）；域级命令分发方案已裁定为 skill 本名（见 §四）。
...
- **会话恢复**：SKILL.md 入口按文件证据（创作进度文件存在性与产物状态）定位续跑点，不凭对话记忆。
