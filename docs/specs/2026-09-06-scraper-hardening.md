...
### 1-4. 四个抓取脚本：头部维护入口注释块 [A]（注释块逐字，插入位置自定）
...
### 5. 四个抓取脚本：空结果显性报错 [B]（文案逐字，分支定位自定）
...
### 6. scan-collection-guide.md：新增「采集礼仪」节 [A]（逐字）
...
- 单次上限：以脚本默认为准（起点单榜 ≤ 100 条、番茄每题材 ≤ 20 本、晋江详情 ≤ 100 本），不手动调大；聚合采集（番茄全题材）总量随题材数放大属正常
...
**偏差分级**：P0 阻塞型（规格与现状冲突，继续施工会做错）→ 立即停下、输出全部改为偏差报告；P1 观察型（有出入不影响按规格施工）→ 继续施工、记入核验请求偏差记录。
