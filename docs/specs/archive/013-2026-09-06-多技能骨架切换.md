...
- [x] `skills/` 下恰有 6 个 skill 目录（abzu-scan + 五占位），`skills/abzu/` 不存在
- [x] 六壳 `agentskills validate` 全过（name 与目录同名）
- [x] 迁移零内容改动：references/scan 10 文件中 9 个 R 100%，scan-workflow.md 为 R 且仅含 §6 两处文案微调；scripts 7 脚本 R 100%
- [x] `node --check` 7/7；grep 断言：四脚本「平台改版维护入口」各=1、「选择器失效」各≥1
- [x] 五占位壳各含「当前尚未建设」；abzu-scan 含「入口判定」「不粘会话」
- [x] `bash scripts/check.sh` 全绿；`npx markdownlint-cli2` 0 违例
- [x] git status 仅含预期 rename/add/modify
...
> 验收完成：2026-09-06，全部通过（规划方核验三步 + 治理文档同步）。D1/D2/D3 偏差详见 collab-log 第 2 行。
