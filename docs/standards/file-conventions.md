...
| 规则 | 内容 |
| --- | --- |
| 退出码 | `0` 成功 / `1` 违规（含运行失败——**产物不可消费**：调用方不得消费产物）/ `2` 参数或环境错误（**条件不满足**：先改条件再重跑）/ `3` 需交互同意但当前为非交互环境 / `4` 用户拒绝或取消（与 `3` 同属需同意情形：`3`＝问不了，`4`＝问了被拒）（**五分类**，缺/空/坏各明示；**`1`／`2` 的判别看调用方动作**；部分成功按 `1` 上报，并在 stderr 明示缺失规模（有原因则逐条列出）。正例 `skills/abzu-analyze/scripts/build-chapter-index.js`——`1`＝数据违规、`2`＝缺参或环境问题；`3`／`4` 的实件为 `skills/abzu-scan/scripts/setup-cdp-chrome.js`） |
| 错误输出 | 错误写 `stderr`（`console.error`），数据写 `stdout`（`console.log`），不混流 |
| 输入校验 | **必填**参数缺失或非法时输出用法说明并退出 `2`；可选参数的默认值须在用法（脚本头注或 `--help`）中标明——不得静默取默认值 |
...
1. **用途**：一行——做什么、给谁用（消费方）。
2. **用法与参数**：调用命令示例 + 参数表（含默认值）；与 §一「输入校验」行（必填缺参→用法说明+退出 2）呼应。
