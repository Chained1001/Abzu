#!/usr/bin/env bash
# 一键检查（提交前必跑）。段序即维护入口——新增检查在此追加一段；段序以本头注为准，其他文档不复述列表；段现状（已实现／待建）亦以本头注段行为准（`# [N]`＝已实现、`# TODO: [N]`＝待建——原《测试与验收标准》§1.1 现状表已随 2026-09-17 重构删除）。行内不写批号——事故与沿革记 `CHANGELOG` 与 git 历史，指认归档规格用全路径。
# 用法：bash scripts/check.sh
# 依赖与前置：Node.js（[1] 段需 npx，未找到即判为失败）；Python 3（[3]／[4]／[5] 段需 python3／python／py，三级链均缺即判为失败）；命令行按 Git Bash 语义执行（Windows）。
#
# [0] skills 目录完整性（安装器 symlink 化检测）——目录未建时跳过
# [1] Markdown 体检（markdownlint）
# [2] skill 格式校验（本地断言：主文件结构／frontmatter／目录形态）
# [3] 内容轨（引用闭合／TOC／行数与条目限长／「维护出处」标注／**每批必读件字数预算**／**用词**（候选；§8 产品领域词豁免））（**加粗密度不设守卫**——阈值已立：`文字与命名标准` §13.8 行数 ÷ 3 保底）——scripts/check_content.py；分档＝可判为失败：行数与条目限长，余项候选只呈报（AGENTS §五.2）
# [4] 规格静态自检（检查 A–E、H–M 十一类；各项判定标准见 scripts/check_spec.py 模块 docstring；本段恒不判为失败，静态发现只呈报）——scripts/check_spec.py --static；非阻断（候选类只呈报）
...
if [ -d skills ]; then
  ...
  # 跳过不静默：原因与手动路径两行**顶格**打印（段输出规范——跳过项要告诉作者手工怎么补跑；
  # 断言判定标准按行首锚定，缩进即假红）；段号登记给末尾收尾汇总（按实况拼，不写死机器状态）。
...
echo "[2] skill 格式校验（本地断言：主文件结构／frontmatter／目录形态）..."
# 纯本地断言、零外部依赖（外部校验器仅人工试跑期可选——其允许清单不含本仓硬约束键）：逐 skills/*/ 校**逐主文件六项**——
...
# `{skill 名}` 同名子目录层与三层深度均违 §1 平铺——二级一并查）。另**段行一条**：⑦**头注段行校验由 `_seg_summary` 统一处理，不在本段逐主文件判**（**段序与段状态按头注取**；本段已由头注改列 0 落地）。
# 计数计算方式：`${#}` 按**字符**计——段首显式 `export LC_ALL=C.UTF-8`（`运行环境标准` §一 未规定 locale；缺省 locale 为 C 时
...
for _d in skills/*/; do
  ...
  # ⑤官方三条约束：name 与目录同名／name ≤64 字符／description 非空且 ≤1024 字符（字符计算方式见段首 locale 注）
  ...
  [ "$_bad" -eq 0 ] && echo "  ${_nm}：逐主文件六项通过"
  skill_bad=$((skill_bad + _bad))
done
if [ "$skill_n" -eq 0 ]; then
  echo "  skills/ 下未落 skill 主文件，跳过"
  echo '手动路径：在 skills/{skill 名}/SKILL.md 落 skill 主文件（形态见 docs/standards/skill资产标准.md §2／§3）'
  seg_note_rt[2]="[2] 无 skill 主文件"
elif [ "$skill_bad" -ne 0 ]; then
  echo "  不通过 $skill_bad 个（共 $skill_n 个 skill 主文件）"
  fail=1
else
  echo "  skill 主文件 $skill_n 个，逐主文件六项全过（⑦段行见头注）"
fi

# Python 三级链探测：python3 → python → py（计算方式见 docs/standards/运行环境标准.md §一.4；顺序不可换）
# 探测上移：[3] 与 [4] 两段**共用**本处结果，各段不再自带第二份探测链
# （`运行环境标准` §一.4「逐脚本不重复实现」）。
PY=""
...
echo "[3] 内容轨（引用闭合／TOC／行数与条目限长／「维护出处」标注／每批必读件字数预算／用词）..."
...
if [ -z "$PY" ]; then echo "x 未找到 Python（探测链 python3 → python → py 均不可用）——本段需要 Python 3"; fail=1
  ...
  # 也不判为失败；静态发现恒 0（非阻断，候选类只呈报），故本段判为失败只可能来自 Python 缺失或工具参数错误
...
_seg_summary() {
  # 收尾汇总：跳过项的可见化写入位置 ＋ 手动路径指引——**顶格打印**（段输出规范：跳过不得静默；
  # 判定标准按行首 `^共 N 段` 锚定，缩进即假红）。段序与待建段按**头注**（段序以头注为准）实况取：`# [N] `
  ...
  if [ ! -r "$_self" ]; then
    # 段序来源不可读：只打显式标记——**不**打错误段数、**不**判为失败
    echo "段序来源不可读（${_self:-未取到路径}）：段数与跳过项未取到，手动路径见各段"
    return
  fi
  while IFS= read -r _l; do
    [ "$_l" = "set -u" ] && break     # 头注止于 `set -u` 行：其后正文的列 0 注释不计成段
    case "$_l" in
