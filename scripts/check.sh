...
# [4] 规格静态自检（断言自噬／计数重算／[A] 逐字／写作检查／三方对账／禁改面禁词核对／自由度分布核对／核销表汇总核对／锚点对源核对）——scripts/check_spec.py --static；非阻断（候选类只呈报，静态发现恒 0）
...
echo "[4] 规格静态自检（断言自噬／计数重算／[A] 逐字／写作检查／三方对账／禁改面禁词核对／自由度分布核对／核销表汇总核对／锚点对源核对）..."
