...
# [3] 内容轨（引用闭合／TOC／行数与条目限长／「维护出处」标注／**热路径字数预算**（候选））（**加粗密度不实现**——阈值未立法，见 `测试与验收标准` §4 G1）——scripts/check_content.py；分档见测试与验收标准 §1.2
# [4] 规格静态自检（检查A 断言自噬／检查B 计数重算／检查C `[A]` 逐字／检查D 写作检查／检查E 三方对账／回写面对账／检查F 禁改面禁词核对／检查G 自由度分布／检查H 核销表汇总核对／仍待项断链核对／检查I 锚点对源核对／检查J 审查记录核对／开放项台账归宿核对／§九 成本字段核对；检查A 的发现项含自噬预警／**零命中待复核**／**零命中核对未判**／**口径不可判**）——scripts/check_spec.py --static；非阻断（候选类只呈报，静态发现恒 0）
...
echo "[3] 内容轨（引用闭合／TOC／行数与条目限长／「维护出处」标注／热路径字数预算）..."
...
echo "[4] 规格静态自检（检查A 断言自噬／检查B 计数重算／检查C [A] 逐字／检查D 写作检查／检查E 三方对账／回写面对账／检查F 禁改面禁词核对／检查G 自由度分布／检查H 核销表汇总核对／仍待项断链核对／检查I 锚点对源核对／检查J 审查记录核对／开放项台账归宿核对／§九 成本字段核对；检查A 的发现项含自噬预警／零命中待复核／零命中核对未判／口径不可判）..."
