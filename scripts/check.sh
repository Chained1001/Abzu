#!/usr/bin/env bash
# 一键检查（提交前必跑）。段序即维护入口——新增检查在此追加一段；段序以本头注为准，其他文档不复述列表；段现状（已实现／待建）亦以本头注段行为准（`# [N]`＝已实现、`# TODO: [N]`＝待建——原《测试与验收标准》§1.1 现状表已随 2026-09-17 重构删除）。行内出现的批次号分三种——**状态型**（书写时一律状态无关）／**判定标准的事故出处**（provenance，保留原文）／**运行时措辞／示例串**（**不得按批次叙事改写**）。
...
# 判为失败接线（093 批 F6④）：工具判为失败（退出 1）与参数／环境错（退出 2）一并接到 fail。
...
# 判为失败接线（093 批 F6④）：语法错与前置缺失均接到 fail。
seg_js=0
...
_seg_summary() {
  # 收尾汇总（078 批 F5）：跳过项的可见化写入位置 ＋ 手动路径指引——**顶格打印**（段输出规范：跳过不得静默；
  ...
  if [ ! -r "$_self" ]; then
    # 段序来源不可读（078 批 P-4）：只打显式标记——**不**打错误段数、**不**判为失败
