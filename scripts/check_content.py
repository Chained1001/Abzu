...
`check_spec.py` 的 `\\|` 静默盲区一件，两件同批处置）。行内出现的批次号分三种——**状态型**（书写时一律状态无关）／**判定标准的事故出处**（provenance，保留原文）／**运行时措辞／示例串**（**不得按批次叙事改写**）。
...
def _check_refs(gov, assets):
    ...
    **候选类**：存量债面宽（`施工机制` §四 分档）。① 的链接扫描**跳过行内代码跨度与
    围栏块**（跨度内与围栏内的示例串不是链接——101 批 F21；判定标准见 `_span_ranges()`／`FENCE_LINE`）。"""
    cands = []
    for path in gov:
