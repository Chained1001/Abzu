...
事故出身与历次裁定：见 `CHANGELOG.md` 对应条目与 `docs/specs/archive/` 各批规格。行内出现的批次号分三种——**状态型**（书写时一律状态无关）／**判定标准的事故出处**（provenance，保留原文）／**运行时措辞／示例串**（**不得按批次叙事改写**）。
...
维护入口：新增断言载体形态扩 ASSERT_CMD 与 _assert_ok()；token 提取与后处理（含 token 反转义）改 _assert_refs()；检查 A 判定标准改 check_swallow()（**候选待办**：`_row_target_text()` 的反转义计算方式未统一——若规格表格与断言命令两处转义写法不一致会失配，触发＝出现实例时同计算方式补反转义；**token 形态三分支**（`^` 剥锚按行首／BRE 元字符不可判／字面计数）的谓词改 `bre_meta`／`literal_token`——**不得复用 `literal_token` 作「不可判」谓词**，见该函数 docstring）；检查 B 的
  ...
  改 check_pairs()／_change_rows()／_target_blocks()／HDR_* 与 DIR_MARKS 常量；检查 C 的成因诊断
   （未命中目标件的引号块 ↔ 改动文件外件集＝`docs/specs/**/*.md` **减本件**，`109` F3）改 _out_of_scope_texts()／
...
# 检查 E（三方核对：改动文件 ↔ 禁改文件 ↔ 断言排除集；077 批 F1）的取集常量——
...
# 检查 F（禁改文件禁词 ↔ 改动清单目标文本的词面命中；089 批 F8）的取集常量——
...
# 与修饰语不紧邻顿号，不入表——过宽即噪声（判定标准收窄，见 089 规格 F8）。
...
def _change_rows(text):
    ...
    路径列＝表头含「文件」或「新产品文档」者（059 的路径列在第 1 列且表头为「新产品文档」）；可自定程度列＝
    表头含「可自定程度」者（无则取末列——059／`施工机制` §八 模板形态）；改动列＝表头含「改动」者（无则取
...
def _read_target(path):
    """**目标件读取计算方式的唯一写入位置**（076 批 F2）：UTF-8 解码 ＋ 对不可解码字节容错（`errors` 取
...
# 改动文件外件集的**去加粗视图**缓存（109 F3）：进程内一次建、跨行跨块复用（`read once` 计算方式见
...
def _out_of_scope_texts(root, spec=None):
    """**改动文件外件集**（`docs/specs/**/*.md` 中**非本件**者，含 `archive/`）的**去加粗**文本视图（109 F3）。
...
def _rel_key(path, root):
    """路径 → **仓根相对、正斜杠**的比对键（109 F3 的件集排除判定标准）。空串入空串。
...
def _out_of_scope_hit(piece, root, spec=None):
    """该引号块是否在**改动文件外件集**中命中（109 F3 的成因诊断判定标准）。命中 ⇒ True。
...
def check_swallow(text, root):
    ...
    **BRE** 行为一致、可严格等价判定；② 剥锚后**仍含 BRE 真正特殊字符**（`. * [ \\ ^ $` 及其反斜杠形态
    ...
    走 ③ 不判（数字无法与命令对齐；`062`:123 那类「分配型＋全同」的既有误报不得复活）。配对支路依赖「数字序＝命令序」
...
def _whole_size(seg, start, unit, line):
    ...
    **不以「加」／「增」收尾**（增量式如「本批给该件加 16 处加粗」不判）时判为整件计算方式；否则降为候选。
    不收窄即生误报（宽松检查关在归档语料上 7 条判定 6 条报到，其中 3 条为误报，实测见 072 §一⑦）；「条」／「项」／
    ...
    for line in text.splitlines():
        ...
        for target, seg in targets:
            if target not in cache:
                ...
                # 被引用件）；读计算方式统一在 `_read_target()`（本处原为就地容错读，075 批 F12）
...
def _strip_wrap(s):
    ...
    实文里是裸反引号——不归一则这类条目在已完工件上恒报误报（050 批五处实测）。另剥加粗标记（规格 A 级行惯以加粗标改动词、实文不含，不剥即恒报未落实误报——101 批 F1 行实例）。**目标侧亦须同等去加粗**（否则恒假阳——109 F1 的写入位置：`check_pairs()` 另取 `content.replace('**','')` 视图并取两视图并集）。"""
    s = s.strip().replace('\\`', '`')
...
def check_pairs(text, root, spec=None):
    ...
    含「可自定程度」（无则取末列，**单元格内含 `[A]` 者视同 `[A]` 行**——059 有「[A]＋[C]」写法）；
    无任何别名可识别的表整表跳过并明示。
    ...
    **最后一个标记之后**的引号块（标记之前的是「现文」＝改后已移除的旧文，核之必误报）；标记之后无
    ...
    ④ **成因诊断（109 F3／事故记录 `#33` 的机械判定标准）**：对**未命中目标件**的引号块，再到**改动文件外件集**
...
def check_anchor(text, root):
    ...
    **判定标准**：① 行号超出目标件实有行数 ② 行号指向空行 ③ 该行±3 内不含规格引文——引文在
    ...
    事故出身：`098`／`099`／`100` 三批成稿审查的位置 P0 族——`100`-P0-1 为**全表行号系前批
...
# 位置一＝条目内**首个非批次号**的反引号跨度：纯数字／批次号在清单全集中恒命中（如 `106`），
...
def check_archive_record(text, root):
    ...
    **静默条件（硬）**：无「审查记录」节**且**各处候选皆空 ⇒ **完全静默**（连汇总行都不打——不误报
    ...
    预估数）。
    事故出身：`091`／`095`／`096`／`097` 四件审查记录节**实质未填**（节内无任何「发现的问题 共 ＋
...
def check_reconcile(text, root):
    ...
    **判二（断言排除集缺）**：E＝验收断言节内 `':!<path>'` 形态（`EXCLUDE_TOKEN`）的路径集
    ...
    # 059 实测 33 行而可解析仅 3 件）；可解析＝C 的件数 N；判一适用＝**命中禁改文件前缀**的件数 K
    ...
    # ＋缺口表，逐面给「已同步更新／无需（理由）」）。**只核该表是否写出，不核结论**——把 `106`／`109`
    # 两批的隐式义务（「哪一面不用同步更新」从未落字）变成显式一行；候选非阻断（`AGENTS.md` §五.2）。
    ...
    # **头注自述只作声明、不作依据**（`105`／`106` 两批「自述豁免失守」的机械化写入位置）。
    n_t = len(targets)
...
def check_ban_words(text, root):
    """检查 F：禁改文件禁词 ↔ 改动清单**目标侧引号块**的词面命中（候选非阻断；089 批 F8）。
...
def static_report(specs, root):
    ...
    **返回值恒为 False**：静态发现（A／B／C／D／E／F／G／H／I／J 十类）一律只呈报、不置退出码（差异⑤：源件对
    ...
    for spec in specs:
        ...
        # 101 批 F26：行收集进 `cell_lines` 并并入 `extra`（计数入末尾三种结果）——修前系裸 print，
    ...
    # 须并入 发现的问题（否则「屏上有行、汇总少计」，`101` F26 同型）
