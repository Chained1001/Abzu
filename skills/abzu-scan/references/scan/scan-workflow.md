...
> **消费点**：进入扫榜调研域（/abzu-scan 或自然语言命中）时全量加载
...
**变量约定**：`{SKILL_DIR}`＝本 skill 根目录；`{扫榜目录}`＝`扫榜/{平台}{方向}_{YYYYMMDD}/`（本次输出目录）；`{平台}`、`{方向}`、`{YYYYMMDD}` 由 Stage 1 交互确定。

**中断与跨域**：用户中途要求停止或表达其他域意图（如扫到一半要写大纲）时，确认当前进度已落盘（或明确放弃未落盘部分）后移交对应域（建议用户使用对应 /abzu-<域> 命令），不强行走完本域流程。
...
- 题材方向（弹窗：选项列表——全榜概览找趋势 / 玄幻 / 都市 / 其他；细分方向、否定表达与跨平台对照意图走「其他」自由输入）：「有没有关注的题材方向？」（原标注「对话式」两轮实测一轮自发弹窗、模态不一致——标注不被遵循则与现实一致优先，2026-09-07 改；素材匹配问句仍为对话式，见 Stage 5）
...
**CDP 依赖提醒**：CDP 模式依赖本地 Chrome 与 agent-browser；启动失败时降级为用户粘贴数据或内置知识模式（排查见 references/scan/scan-cdp-base.md）。

**数据不合格分支**：采集标 `[存在问题]`、有效条目 < 5〔abzu 自定〕、或用户粘贴数据无法解析时，不进入 Stage 3——向用户说明问题并给选项（AskUserQuestion 选项：重新采集 / 改用粘贴数据 / 降级内置知识）。

### Stage 3：数据分析

**优先运行** `node {SKILL_DIR}/scripts/scan-analyze.js --dir {扫榜目录} [--genre {题材}] [--dup] [--full]` 做确定性提取（脚本非零退出时不得继续分析——向用户报告错误输出后排查，不吞退出码），**禁止临时写内联解析脚本**（AI 手写 grep 会把「玄幻·东方玄幻」重复计数；脚本按条目计数）。4 平台通用提取（按文件头自动识别）：全平台可提取排名/书名/作者；起点字段最全；番茄提取字数/在读/题材/标签；晋江提取收藏/字数（题材固有缺失标 `[待补]`）；七猫提取热度/字数/题材。`--dup` 跨平台聚合（同一本书多平台上榜 = 交叉验证信号）。刺猬猫不在支持范围。
...
把扫榜结果变成能直接用的选题建议，产出**本次扫榜输出目录** `{扫榜目录}/选题决策.md`。选题方法（选题四步 + 可行性判断）见 references/scan/scan-topic-decision.md；落盘模板（字段定义/默认值/示例）见 references/scan/scan-topic-decision-template.md。收尾只问一个决策（AskUserQuestion 选项：候选选题 + 「都不满意」）：「你倾向哪个选题？」选定后针对该题问素材匹配（对话式：询问是否有设定、灵感、参考作品等可纳入选题的积累），选定与素材结论记入选题决策.md「终裁记录」节（模板可选节，写法决策与翻转条件按 scan-topic-decision.md 收尾步骤商定）；**不问计划字数**（平台+方向已定则篇幅由市场定义）。收尾告知产物路径即完成，后续去向由用户决定（跨域走对应域命令）。
