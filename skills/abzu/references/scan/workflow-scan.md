...
> **来源**：移植自 mo-shu v2.6.1（moshu-scan/SKILL.md v1.2.0），适配 abzu 单 skill 架构
> **边界**：本文件管扫榜域全流程；CDP 环境操作见 [cdp-base.md](cdp-base.md)；选题决策产物供未来立项域消费
...
**脚本采集模式**（各平台榜单表、命令示例、输出目录约定、采集质量检查四步）见 [collection-guide.md](collection-guide.md)。要点：起点默认移动端 SSR 不需 Chrome；番茄/七猫/晋江按需启动 CDP 底座（[cdp-base.md](cdp-base.md)）；输出规范与字段定义见 [scan-output-format.md](scan-output-format.md)。
...
各平台分析维度与通用维度清单见 [analysis-guide.md](analysis-guide.md)。晋江数据若标 `[仅列表-无核心指标]` 视为不合格，不足以支撑分析。

### Stage 4：输出扫榜报告

报告模板见 [analysis-guide.md](analysis-guide.md)「扫榜报告模板」节，写入 `{扫榜目录}/扫榜报告_{平台}{方向}_{YYYYMMDD}.md`。

### Stage 5：选题决策

把扫榜结果变成能直接用的选题建议，产出**本次扫榜输出目录** `{outdir}/选题决策.md`。完整方法（选题四步 + 可行性判断 + 输出模板）见 [topic-decision.md](topic-decision.md)。收尾只问一个决策：「倾向哪个选题，或都不满意？」选定后针对该题问素材匹配；**不问计划字数**（平台+方向已定则篇幅由市场定义）；拆文验证保持用户独立决定，不自动衔接。
...
| 文件 | 何时加载 |
| --- | --- |
| [collection-guide.md](collection-guide.md) | Stage 2 采集：平台榜单表/命令示例/输出目录约定/质量检查四步/核心哲学三原则 |
| [analysis-guide.md](analysis-guide.md) | Stage 3/4：各平台分析维度 + 通用维度 + 扫榜报告模板 |
| [topic-decision.md](topic-decision.md) | 「选题决策」：选题四步 + 可行性判断 + 选题决策.md 模板 |
| [reader-profiling.md](reader-profiling.md) | 需要分析目标读者画像时 |
| [genre-trends.md](genre-trends.md) | 查看题材趋势候选、切入约束和样本校验规则时 |
| [publishing-guide.md](publishing-guide.md) | 平台适配+推荐机制校验+数据指标+简介设计 |
| [scan-output-format.md](scan-output-format.md) | 脚本/CDP 采集字段定义+输出模板 |
| [cdp-base.md](cdp-base.md) | CDP 环境启动与浏览器操作（番茄/七猫/晋江需要时） |
...
| `{SKILL_DIR}/scripts/setup-cdp-chrome.js` | CDP Chrome 启动器（用法见 cdp-base.md） |
