...
把扫榜结果变成能直接用的选题建议，产出**本次扫榜输出目录** `{outdir}/选题决策.md`。完整方法（选题四步 + 可行性判断 + 输出模板）见 [topic-decision.md](topic-decision.md)。收尾只问一个决策（弹窗：候选选题 + 「都不满意」）：「你倾向哪个选题？」选定后针对该题问素材匹配（对话式）；**不问计划字数**（平台+方向已定则篇幅由市场定义）；拆文验证保持用户独立决定，不自动衔接。
